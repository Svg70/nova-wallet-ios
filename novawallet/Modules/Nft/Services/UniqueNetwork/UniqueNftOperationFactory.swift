...
final class UniqueNftOperationFactory: UniqueNftOperationFactoryProtocol {
    ...
    func fetchNfts(
        ...
        print(
            "🔍 [UniqueNftOperationFactory] fetchNfts for address:",
            address,
            "offset:",
            offset,
            "limit:",
        ...
        networkOperation.completionBlock = { [weak networkOperation] in
            guard let op = networkOperation else { return }
            switch op.result {
            case let .success(response):
                print("✅ [UniqueNftOperationFactory] response count:", response.items.count)
            case let .failure(error):
                if !NetworkOperationHelper.isCancellation(error: error) {
                    print("❌ [UniqueNftOperationFactory] error:", error)
                }
            case .none:
                break
            }
