...
final class UniqueDetailsInteractor: NftDetailsInteractor {
    ...
    private func provideMedia(from imageUrlString: String?) {
        ...
        if let urlString = imageUrlString, !urlString.isEmpty, let directUrl = URL(string: urlString) {
            ...
            // } else {
            //     print("⚠️ [UniqueDetailsInteractor] URL scheme is not http/https: \(urlString)")
            //     mediaViewModel = nil
            // }
        } else {
            if let urlString = imageUrlString, !urlString.isEmpty {
                print("⚠️ [UniqueDetailsInteractor] Invalid URL string for media: \(urlString)")
            }
            mediaViewModel = nil
        }
